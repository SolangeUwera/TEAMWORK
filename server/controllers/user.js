...
export const signup = (req, res) => {
  const {
    firstName, lastName, email, password, Gender, JobRole, Department, Adress,
  } = req.body;
  ...
  const newUser = {
    id: userId, password: hashedPassword, firstName, lastName, email, Gender, JobRole, Department, Adress,
  };
  ...
  return res.status(201).send({
    ...
    data: {
      id: userId, firstName, lastName, email, Gender, JobRole, Department, Adress, Xtoken,
    },
  });
};
export const signin = (req, res) => {
  ...
  return res.status(200).send({
    status: 200,
    message: 'user is sucessfully Loged in ',
    info: {
      id: Data.id,
